...
def loss_report(x: np.array, y: np.array, lst_models: list):
    ...
    print('\n' + '#' * 6 + ' Score report ' + '#' * 5 + '| MSE:')
    for model in lst_models:
        loss = model.loss_(x[:, data_idx(model._vars_)], y)
...
if __name__ == "__main__":
    ...
    # ###################################################################### #
    # Simple models:
    lr_w = MyLR(np.random.rand(2, 1), alpha=1e-2, max_iter=nb_steps)
    lr_p = MyLR(np.random.rand(2, 1), alpha=1e-2, max_iter=nb_steps)
    lr_t = MyLR(np.random.rand(2, 1), alpha=1e-2, max_iter=nb_steps)

    lr_wp = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)
    lr_wt = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)
    lr_pt = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)

    lr_wpt = MyLR(np.random.rand(4, 1), alpha=1e-2, max_iter=nb_steps)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_use) as executor:
        ...
        for ii, model, vars in zip(range(nb), simple_models, lst_vars1):
            print(f"Batch simple: starting model {ii + 1} / {nb}",
                  end='\r',
                  flush=True)
    ...
    # ###################################################################### #
    # 'intermediate' models
    lr_w2 = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w3 = MyLR(np.random.rand(4, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w4 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)

    lr_p2 = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)
    lr_p3 = MyLR(np.random.rand(4, 1), alpha=1e-2, max_iter=nb_steps)
    lr_p4 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)

    lr_t2 = MyLR(np.random.rand(3, 1), alpha=1e-2, max_iter=nb_steps)
    lr_t3 = MyLR(np.random.rand(4, 1), alpha=1e-2, max_iter=nb_steps)
    lr_t4 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)

    lr_w_p_2 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w_p_3 = MyLR(np.random.rand(7, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w_p_4 = MyLR(np.random.rand(9, 1), alpha=1e-2, max_iter=nb_steps)

    lr_w_t_2 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w_t_3 = MyLR(np.random.rand(7, 1), alpha=1e-2, max_iter=nb_steps)
    lr_w_t_4 = MyLR(np.random.rand(9, 1), alpha=1e-2, max_iter=nb_steps)

    lr_p_t_2 = MyLR(np.random.rand(5, 1), alpha=1e-2, max_iter=nb_steps)
    lr_p_t_3 = MyLR(np.random.rand(7, 1), alpha=1e-2, max_iter=nb_steps)
    lr_p_t_4 = MyLR(np.random.rand(9, 1), alpha=1e-2, max_iter=nb_steps)

    intermediate_models = [lr_w2, lr_w3, lr_w4, lr_p2, lr_p3, lr_p4, lr_t2,
                           lr_t3, lr_t4, lr_w_p_2, lr_w_p_3, lr_w_p_4,
                           lr_w_t_2, lr_w_t_3, lr_w_t_4, lr_p_t_2, lr_p_t_3,
                           lr_p_t_4]
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_use) as executor:
        for ii, model, vars in zip(range(nb), intermediate_models, lst_vars2):
            print(f"Batch intermediate: starting model {ii + 1} / {nb}",
                  end='\r',
                  flush=True)
        ...
        for task in concurrent.futures.as_completed(batch2_future):
            if task.result() is not None:
                ...
                s_state[task.result()._idx_] = '[✔]'
                print('Intermediate batch: ' + ' '.join(s_state),
                      end='\r',
                      flush=True)
    ...
    lr_WP_t1 = MyLR(np.random.rand(10, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WP_t2 = MyLR(np.random.rand(11, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WP_t3 = MyLR(np.random.rand(12, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WP_t4 = MyLR(np.random.rand(13, 1), alpha=1e-2, max_iter=nb_steps)

    # models with ∑w^n n in (1,...,4) and ∑t^n n in (1,...,4) plus
    # the terms ∑p^m. 1st: p^1 - 2nd: p^1 + p^2 - 3rd: p^1 + p^2 + p^3
    lr_WT_p1 = MyLR(np.random.rand(10, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WT_p2 = MyLR(np.random.rand(11, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WT_p3 = MyLR(np.random.rand(12, 1), alpha=1e-2, max_iter=nb_steps)

    # models with ∑p^n n in (1,...,4) and ∑t^n n in (1,...,4) plus
    # the terms ∑w^m. 1st: w^1 - 2nd: w^1 + w^2 - 3rd: w^1 + w^2 + w^3
    lr_PT_w1 = MyLR(np.random.rand(10, 1), alpha=1e-2, max_iter=nb_steps)
    lr_PT_w2 = MyLR(np.random.rand(11, 1), alpha=1e-2, max_iter=nb_steps)
    lr_PT_w3 = MyLR(np.random.rand(12, 1), alpha=1e-2, max_iter=nb_steps)
    ...
    lr_WPT_wp = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_w2p = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_wp2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_w2p2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_WP = MyLR(np.random.rand(17, 1), alpha=1e-2, max_iter=nb_steps)

    # models with ∑(w^n+ p^n + t^n) n in (1,...,4) plus
    # one cross term: 'wt', w^2t, wt^2, w^2t^2
    # and finally all the cross terms 'wt' + w^2t + wt^2 + w^2t^2
    lr_WPT_wt = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_w2t = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_wt2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_w2t2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_WT = MyLR(np.random.rand(17, 1), alpha=1e-2, max_iter=nb_steps)

    # models with ∑(w^n+ p^n + t^n) n in (1,...,4) plus
    # one cross term: 'pt', p^2t, pt^2, p^2t^2
    # and finally all the cross terms 'pt' + p^2t + pt^2 + p^2t^2
    lr_WPT_pt = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_p2t = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_pt2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_p2t2 = MyLR(np.random.rand(14, 1), alpha=1e-2, max_iter=nb_steps)
    lr_WPT_PT = MyLR(np.random.rand(17, 1), alpha=1e-2, max_iter=nb_steps)

    # models with ∑(w^n+ p^n + t^n) n in (1,...,4) plus
    # all the cross term of wp, wt and pt
    lr_WPT_WP_WT_PT = MyLR(np.random.rand(25, 1), alpha=1e-2, max_iter=nb_steps)

    complex_models = [lr_WP_t1, lr_WP_t2, lr_WP_t3, lr_WP_t4,
    ...
    batch3_trained = []
    batch3_future = []
    nb = len(complex_models)
    s_state = ['[ ]'] * nb
    print('\n')
    with concurrent.futures.ProcessPoolExecutor(max_workers=cpu_use) as executor:
        for ii, model, vars in zip(range(nb), complex_models, lst_vars3):
            print(f"Batch 'complex': starting model {ii + 1} / {nb}",
                  end='\r',
                  flush=True)
        ...
        for task in concurrent.futures.as_completed(batch3_future):
            if task.result() is not None:
                ...
                s_state[task.result()._idx_] = '[✔]'
                print("complex batch: " + ' '.join(s_state),
                      end='\r',
                      flush=True)
