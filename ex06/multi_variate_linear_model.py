...
if __name__ == '__main__':
    ...
    np.random.seed = 42
    mylr_Age = MyLinearRegression(np.random.rand(2, 1),
                                  alpha=5e-3, max_iter=50000)
    mylr_Thrust = MyLinearRegression(np.random.rand(2, 1),
                                     alpha=1e-4, max_iter=50000)
    mylr_Distance = MyLinearRegression(np.random.rand(2, 1),
                                       alpha=2e-4, max_iter=100000)
    ...
    axes_labels = [r"$x_3$: distance totalizer of spacecraft (in Tmeters)",
                   "y: sell price (in keuros)"]
    ...
    # MSE repport:
    print("#" * 50, "\nLoss report:")
    lst_s_feats = ["Age", "Thrust", "Distance"]
    lst_mse = [mse_Age, mse_Thrust, mse_Distance]
    for feat, mse in zip(lst_s_feats, lst_mse):
    ...
    # ############################################################### #
    n_feat = 3
    mylr_multi = MyLinearRegression(np.random.rand(n_feat + 1, 1),
                                    alpha=5e-5,
                                    max_iter=75000)
