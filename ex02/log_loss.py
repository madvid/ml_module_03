...
def log_loss_(y, y_hat, eps=1e-18):
    ...
    try:
        ...
        if (y.ndim != 2) or (y_hat.ndim != 2) \
            ...
            s = "y or y_hat not 2 dimensional array " \
                + "or mismatching shape between y and y_hat"
            print(s, file=sys.stderr)
            return None

        log_loss = 0
        for y_ii, yhat_ii in zip(y, y_hat):
            log_loss += y_ii * np.log(yhat_ii + eps) \
                      + (1 - y_ii) * np.log(1 - yhat_ii + eps)
